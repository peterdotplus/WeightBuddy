...
const mockedSelectRandomCategory = selectRandomCategory as jest.MockedFunction<typeof selectRandomCategory>;
const mockedGetCategoryPrompt = getCategoryPrompt as jest.MockedFunction<typeof getCategoryPrompt>;
const mockedGenerateInspirationMessage = generateInspirationMessage as jest.MockedFunction<typeof generateInspirationMessage>;
...
describe('Inspiration Service', () => {
  ...
  describe('sendDailyInspiration', () => {
    it('should send a motivation message when motivation category is selected', async () => {
      ...
      mockedGetCategoryPrompt.mockReturnValue('Provide a motivational message for weight loss journey');
      mockedGenerateInspirationMessage.mockResolvedValue('You are doing great! Keep going!');
      ...
      expect(mockedGenerateInspirationMessage).toHaveBeenCalledWith(
        'Provide a motivational message for weight loss journey'
      );

      // Verify Telegram message sending
      expect(mockedSendTelegramMessage).toHaveBeenCalledWith('You are doing great! Keep going!');
    });
    ...
    it('should send a check-in message when check-in category is selected', async () => {
      ...
      mockedGetCategoryPrompt.mockReturnValue('Ask a check-in question about weight loss progress');
      mockedGenerateInspirationMessage.mockResolvedValue('How has your progress been this week?');
      mockedSendTelegramMessage.mockResolvedValue();
      ...
      // Verify AI message generation
      expect(mockedGenerateInspirationMessage).toHaveBeenCalledWith(
        'Ask a check-in question about weight loss progress'
      );

      // Verify Telegram message sending
      expect(mockedSendTelegramMessage).toHaveBeenCalledWith('How has your progress been this week?');
    });
    ...
    it('should handle errors from AI service gracefully', async () => {
      ...
      mockedGenerateInspirationMessage.mockRejectedValue(new Error('AI Service Error'));

      await expect(sendDailyInspiration()).rejects.toThrow('Failed to send daily inspiration: AI Service Error');
    ...
    it('should handle errors from Telegram service gracefully', async () => {
      ...
      mockedSelectRandomCategory.mockReturnValue('Motivation');
      mockedGetCategoryPrompt.mockReturnValue('Provide a motivational message');
      mockedGenerateInspirationMessage.mockResolvedValue('You are doing great!');
      mockedSendTelegramMessage.mockRejectedValue(new Error('Telegram API Error'));

      await expect(sendDailyInspiration()).rejects.toThrow('Failed to send daily inspiration: Telegram API Error');
    });
    ...
    it('should return the sent message for logging purposes', async () => {
      ...
      mockedGenerateInspirationMessage.mockResolvedValue('How are you feeling today?');
      mockedSendTelegramMessage.mockResolvedValue();
